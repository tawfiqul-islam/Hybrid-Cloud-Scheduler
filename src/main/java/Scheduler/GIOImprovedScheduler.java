...
public class GIOImprovedScheduler extends Thread {
    ...
    public void run() {
        ...
        while(true) {
                        ...
                        for (int i = 0; i < SchedulerUtil.jobQueue.size(); i++) {
                            ...
                            if (currentJob.isShutdown()) {
                                ...
                                if (placeExecutor(currentJob,this.getClass())) {
                                    ...
                                    Log.SchedulerLogging.log(Level.INFO, GIOImprovedScheduler.class.getName() + ": Placed executor(s) for Job: " + currentJob.getJobID());
                                    ...
                                    if (currentJob.getAllocatedExecutors() == currentJob.getExecutors()) {
                                        Log.SchedulerLogging.log(Level.INFO, GIOImprovedScheduler.class.getName() + ": All executors are placed for Job: " + currentJob.getJobID());
                                        //remove job from job queue
                                        SchedulerUtil.jobQueue.remove(i);
                                        Log.SchedulerLogging.log(Level.INFO, GIOImprovedScheduler.class.getName() + ": Removed Job: " + currentJob.getJobID() + " from jobQueue");
                                        //add job to fully submitted job list
                                        SchedulerUtil.fullySubmittedJobList.add(currentJob);
                                        Log.SchedulerLogging.log(Level.INFO, GIOImprovedScheduler.class.getName() + ": Added Job: " + currentJob.getJobID() + " to fullySubmittedJobList");
                                        i--;
                                    }
                                } else {
                                    //could not place any executors for the current job
                                    Log.SchedulerLogging.log(Level.INFO, GIOImprovedScheduler.class.getName() + ":Could not place any executor(s) for Job: " + currentJob.getJobID());
                                }
                            }
                            ...
                            for(int j=0;j<SchedulerUtil.fullySubmittedJobList.size();j++)
                            {
                                ...
                                if (!currentSubmittedJob.isSubmitted()&&currentSubmittedJob.isResourceReserved()) {
                                    ...
                                    Log.SchedulerLogging.log(Level.INFO, GIOImprovedScheduler.class.getName() + ": Submitting Job: " + currentSubmittedJob.getJobID() +" with role: "+currentSubmittedJob.getRole()+ " to the Cluster");
            ...
            if (shutdown&&(SchedulerUtil.fullySubmittedJobList.size()==0||(System.currentTimeMillis()-shutdownJobArrivalTime)/1000>=1200)) {
                Log.SchedulerLogging.log(Level.INFO, GIOImprovedScheduler.class.getName() + "Shutting Down GIOHybrid Scheduler. Job Queue is Empty...");
    ...
    static boolean placeExecutor(Job currentJob, Class classVar)  {
        ...
        double oldPredictedTime=currentJob.getTotalPredictedTime();
        boolean localOnly=false;
        if(findLocalPlacement(currentJob,classVar))
        {
            localOnly=true;
        }
        else
        {
            currentJob.setTotalPredictedTime(currentJob.getTotalPredictedTime()+currentJob.getTotalPredictedTime()* SchedulerUtil.networkPenalty);
        }
        while(true) {
            ...
            for(int i=0;i<SchedulerUtil.agentList.size();i++) {
                ...
                if(localOnly)
                {
                    if(!SchedulerUtil.agentList.get(i).isLocal())
                        continue;
                }
        ...
        //Also set the initial position of the index
            ...
            currentJob.setTotalPredictedTime(oldPredictedTime);
    ...
    static boolean findLocalPlacement(Job currentJob, Class classVar)
    {
        int execpossiblecount=0;
        for(int i=0;i<SchedulerUtil.agentList.size();i++) {
            if(!SchedulerUtil.agentList.get(i).isLocal())
                continue;
            execpossiblecount+=(int)Math.min(Math.floor(SchedulerUtil.agentList.get(i).getCpu()/currentJob.getCoresPerExecutor()),Math.floor(SchedulerUtil.agentList.get(i).getMem()/currentJob.getMemPerExecutor()));
        }

        if(currentJob.getExecutors()<=execpossiblecount) {
            Log.SchedulerLogging.log(Level.INFO, GIOImprovedScheduler.class.getName() + ": For Job: " + currentJob.getJobID() + " local-only placement is possible");
            return true;
        }
        else {
            Log.SchedulerLogging.log(Level.INFO, GIOImprovedScheduler.class.getName() + ": For Job: " + currentJob.getJobID() + " local-only placement is NOT possible");
            return false;
        }
    }
}
