...
public class AsQScheduler extends Thread {
    ...
    public void run() {
        ...
        while(true) {
                        ...
                        for (int i = 0; i < SchedulerUtil.jobQueue.size(); i++) {
                            ...
                            if (currentJob.isShutdown()) {
                                ...
                                if (placeExecutor(currentJob,this.getClass())) {
                                    ...
                                    Log.SchedulerLogging.log(Level.INFO, AsQScheduler.class.getName() + ": Placed executor(s) for Job: " + currentJob.getJobID());
                                    ...
                                    if (currentJob.getAllocatedExecutors() == currentJob.getExecutors()) {
                                        Log.SchedulerLogging.log(Level.INFO, AsQScheduler.class.getName() + ": All executors are placed for Job: " + currentJob.getJobID());
                                        //remove job from job queue
                                        SchedulerUtil.jobQueue.remove(i);
                                        Log.SchedulerLogging.log(Level.INFO, AsQScheduler.class.getName() + ": Removed Job: " + currentJob.getJobID() + " from jobQueue");
                                        //add job to fully submitted job list
                                        SchedulerUtil.fullySubmittedJobList.add(currentJob);
                                        Log.SchedulerLogging.log(Level.INFO, AsQScheduler.class.getName() + ": Added Job: " + currentJob.getJobID() + " to fullySubmittedJobList");
                                        i--;
                                    }
                                } else {
                                    //could not place any executors for the current job
                                    Log.SchedulerLogging.log(Level.INFO, AsQScheduler.class.getName() + ":Could not place any executor(s) for Job: " + currentJob.getJobID());
                                }
                            }
                            ...
                            for(int j=0;j<SchedulerUtil.fullySubmittedJobList.size();j++)
                            {
                                ...
                                if (!currentSubmittedJob.isSubmitted()&&currentSubmittedJob.isResourceReserved()) {
                                    ...
                                    Log.SchedulerLogging.log(Level.INFO, AsQScheduler.class.getName() + ": Submitting Job: " + currentSubmittedJob.getJobID() +" with role: "+currentSubmittedJob.getRole()+ " to the Cluster");
            ...
            if (shutdown&&(SchedulerUtil.fullySubmittedJobList.size()==0||(System.currentTimeMillis()-shutdownJobArrivalTime)/1000>=1200)) {
                Log.SchedulerLogging.log(Level.INFO, AsQScheduler.class.getName() + "Shutting Down GIOHybrid Scheduler. Job Queue is Empty...");
    ...
    static boolean findLocalPlacement(Job currentJob, Class classVar)
    {
        ...
        for(int i=0;i<SchedulerUtil.agentList.size();i++) {
            if(!SchedulerUtil.agentList.get(i).isLocal())
                continue;
            execpossiblecount+=(int)Math.min(Math.floor(SchedulerUtil.agentList.get(i).getCpu()/currentJob.getCoresPerExecutor()),Math.floor(SchedulerUtil.agentList.get(i).getMem()/Math.ceil(currentJob.getTotalExecutorMemory())));
        }

        if(currentJob.getExecutors()<=execpossiblecount) {
            Log.SchedulerLogging.log(Level.INFO, AsQScheduler.class.getName() + ": For Job: " + currentJob.getJobID() + " local-only placement is possible");
            return true;
        }
        else {
            Log.SchedulerLogging.log(Level.INFO, AsQScheduler.class.getName() + ": For Job: " + currentJob.getJobID() + " local-only placement is NOT possible");
            return false;
        }
    }
}
